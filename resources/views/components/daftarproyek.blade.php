@props(['proyek'])
<div id="projectModal" class="z-100 hidden fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center opacity-0 transition-opacity duration-500">
    <div id="modalContent" class="bg-white p-8 rounded-lg w-3/4 md:w-1/2 transform -translate-y-full opacity-0 transition-all duration-500 ease-out relative shadow-2xl">
        <!-- Tombol Close (X) -->
        ...
        <!-- Konten Modal -->
        <h2 class="text-3xl font-semibold mb-6 text-center text-primary">Daftar Proyek</h2>
        ...
        <form action="{{ route('pendaftaran', ['proyek' => $proyek->id]) }}" method="POST" class="space-y-8">
            ...
            <!-- Persyaratan Kemampuan (Checkbox) -->
            <div>
            ...
            <div class="flex justify-center">
                <button type="submit" class="w-full py-3 mt-4 bg-primary text-white rounded-lg shadow-lg hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50 transition-all duration-300">
                    Daftar
                </button>
            </div>
        </form>
