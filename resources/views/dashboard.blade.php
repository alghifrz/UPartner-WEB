        ...
        <div class="py-8 sm:py-10 lg:py-12 flex flex-col-reverse lg:flex-row justify-between items-center max-w-[1500px] mx-auto mb-12 px-4 sm:px-6 lg:px-8">
            <div class="max-w-[800px] mb-8 lg:mb-0 text-center lg:text-left">
                ...
                <div class="flex justify-center lg:justify-start">
                    <a href="{{ route('katalog') }}" class="text-white text-lg sm:text-2xl font-semibold rounded-full bg-secondary hover:bg-primary py-3 px-8">{!! $dashboard['quotes']['button'] !!}</a>
                </div>
            </div>
            ...
            <div class="mx-8 md:mx-12 lg:mx-24 flex flex-wrap justify-center gap-8 md:gap-16 z-50 relative">
                @foreach($topStudents as $index => $student)
                    @if ($student->id == Auth::user()->id)
                        <a href="{{ route('profile.edit') }}" class="w-full sm:w-[45%] md:w-[30%] lg:w-[25%] cursor-pointer bg-white bg-opacity-70 backdrop-blur-md rounded-2xl p-6 text-center shadow-lg hover:scale-105 hover:duration-500 transition-transform border-4 border-gray-200">
                    ...
                    @else
                        <a href="{{ route('lihatprofil', $student) }}" class="w-full sm:w-[45%] md:w-[30%] lg:w-[25%] cursor-pointer bg-white bg-opacity-70 backdrop-blur-md rounded-2xl p-6 text-center shadow-lg hover:scale-105 hover:duration-500 transition-transform">
                            <div class="relative mb-4">
                                @if($index == 0)
                                    <span class="absolute top-0 left-0 bg-yellow-500 text-white px-3 py-1 rounded-full text-sm font-bold">1st</span>
                                @elseif($index == 1)
