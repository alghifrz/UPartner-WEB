<x-dosen-app-layout :title="'Katalog'" :footer="$footer">
    <x-slot name="katalog">
        ...
        <form action="{{ route('dosen.search') }}" method="GET" class="flex sm:flex-row justify-center items-center gap-0 mx-12 mb-8 mt-12">
            <input 
                type="text" 
                name="keyword" 
    ...
    <div class="max-w-[1500px] mx-auto px-4 sm:px-6 md:px-6 lg:px-8 mt-12">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse ($proyek as $proyekItem)
                <x-cardproyek :proyek="$proyekItem" :detail="route('dosen.detailproyek', $proyekItem)" />
            @empty
                <div class="w-full col-span-full flex justify-center">
                    <div class="w-full col-span-full flex justify-center flex-col items-center text-center">
                        ...
                        <a href="{{ route('dosen.katalog') }}" class="bg-secondary text-white py-2 px-6 rounded-lg font-semibold hover:bg-primary transition">
                            Lihat Semua Proyek
                        </a>
                    </div>
                </div>
