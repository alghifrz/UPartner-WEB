<x-layoutguest :title="'Katalog'" :footer="$footer">
    <x-slot name="katalog">
        ...
        <form action="{{ route('searchguest') }}" method="GET" class="flex sm:flex-row justify-center items-center gap-0 mx-12 mb-8 mt-12">
            <input 
                type="text" 
                name="keyword" 
                value="{{ request('keyword') }}"
                placeholder="Cari proyek sesuai judul, deskripsi, atau nama dosen..." 
                class="w-full sm:w-2/3 lg:w-1/3 px-6 py-3 border border-white rounded-l-2xl text-gray-800 text-lg"
            />
            <button 
                type="submit" 
    ...
    <div class="max-w-[1500px] mx-auto px-4 sm:px-6 md:px-6 lg:px-8 mt-12">
        <div class="flex justify-between items-center mb-2">
            ...
            <div id="filterModal" class="fixed inset-0 bg-black bg-opacity-50 justify-center items-center hidden z-50 overflow-hidden">
                <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg w-full max-w-[90%] sm:w-1/2 lg:w-1/3 p-6 px-4 mx-auto sm:mx-8 overflow-hidden">
                    ...
                    <form action="{{ route('katalogguest') }}" method="GET">
                        <div class="flex flex-col space-y-4 overflow-y-auto max-h-[70vh] p-4">
                            <label class="flex items-center">
                                <input 
                                    ...
                                    @if(empty(request('program_studi'))) checked @endif>
                                Pilih Semua
                            </label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            @forelse ($proyek as $proyekItem)
                <x-cardproyek :proyek="$proyekItem" :detail="route('detailproyekguest', $proyekItem)" />
            @empty
                <div class="w-full col-span-full flex justify-center">
                    <div class="w-full col-span-full flex justify-center flex-col items-center text-center">
                        ...
                        <a href="{{ route('katalogguest') }}" class="bg-secondary text-white py-2 px-6 rounded-lg font-semibold hover:bg-primary transition">
                            Lihat Semua Proyek
                        </a>
                    </div>
                </div>
